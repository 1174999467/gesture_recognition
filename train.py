...
# 首先要读取图片信息并导入。
PATH = 'G:\PyCharmProject\gesture_recognition\img'# 所有的手势图片都放在里面
img_rows = 300
imgs_cols = 300
img_channels = 1
batch_size = 32
...
MODEL_SAVE_PATH = 'G:\\PyCharmProject\\gesture_recognition\\model'
...
def Initializer():
    ...
    data, label = utils.shuffle(immatrix, label, random_state=0)
    X_train, X_test, y_train, y_test = model_selection.train_test_split(data, label, test_size=0.2, random_state=4)
    X_train = X_train.reshape(X_train.shape[0], img_rows, imgs_cols, img_channels) #  tensorflow的图像格式为[batch, W, H, C]
    X_test = X_test.reshape(X_test.shape[0], img_rows, imgs_cols, img_channels) #  tensorflow的图像格式为[batch, W, H, C]
...
def train(X_train, y_train):
    x = tf.placeholder(tf.float32, [batch_size, img_rows, imgs_cols, img_channels], name='x-input')
    ...
    learninig_rate = tf.train.exponential_decay(LEARNING_RATE_BASE, global_step, 1200//batch_size, LEARNING_RATE_DECAY)
    train_step = tf.train.GradientDescentOptimizer(learninig_rate).minimize(loss, global_step=global_step)
...
def test(X_test, y_test):
    ...
    with tf.Graph().as_default() as g: # 设置默认graph
        # 定义输入输出格式
        #
        x = tf.placeholder(tf.float32, [1, img_rows, imgs_cols, img_channels], name='x-input')
